...
export const updateNote = async(req, res)=>{
  try {
    const {title, content} =  req.body;
    const updatedNote = await NoteSchema.findByIdAndUpdate(req.params.id, {title,content}, {new:true})
    if(!updatedNote){
...
try {
  const deleteNote = await NoteSchema.findByIdAndDelete(req.params.id)
  if(!deleteNote){
